...
class FCNs(nn.Module):
    ...
    def forward_features(self, x3, x4, x5):
        """
        Forward pass with pre-computed features (for feature extraction training)

        Args:
            x3: Features from pool3 (N, 256, H/8, W/8)
            x4: Features from pool4 (N, 512, H/16, W/16)
            x5: Features from pool5 (N, 512, H/32, W/32)

        Returns:
            score: Segmentation output (N, n_class, H, W)
        """
        score = self.relu(self.bn1(self.deconv1(x5)))     # size=(N, 512, x.H/16, x.W/16)
...
class FCNsResNet(nn.Module):
    ...
    def forward_features(self, x1, x2, x3, x4):
        """
        Forward pass with pre-computed features (for feature extraction training)

        Args:
            x1: Features from layer1 (N, 256, H/4, W/4)
            x2: Features from layer2 (N, 512, H/8, W/8)
            x3: Features from layer3 (N, 1024, H/16, W/16)
            x4: Features from layer4 (N, 2048, H/32, W/32)
